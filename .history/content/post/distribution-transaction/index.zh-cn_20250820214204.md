...
#### 可靠消息需要解决的核心问题
  ...
  - 事务发起方提交事务成功
    - 投递消息至消息中间件一定能投递成功否则事务发起方一定能回滚事务
    - 事务参与方一定能从消息中间件接收到消息并能处理成功否则事务发起方一定能回滚事务
  
  - 事务发起方提交事务失败
    - 消息一定不会被事务参与方消费
  
  可见，可靠消息分布式事务一共包含了四步操作：
  ...
  在可靠消息分布式事务中一共包含了三个角色：事务发起方、消息中间件、事务参与方法，各个角色需要保证：
...
- 先发消息
   ...
   - 执行事务时发送消息：在写库完成前发送消息可能会出现消息发送成功，但写库失败的场景。

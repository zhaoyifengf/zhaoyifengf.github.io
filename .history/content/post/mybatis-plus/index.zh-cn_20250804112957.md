...
sequenceDiagram
    ...
    participant DataSourceTransactionManager
    participant TransactionSynchronizationManager
    participant DataSource
    participant SqlSession
    participant BatchExecutor
    
    Client->>TransactionInterceptor: 调用@Transactional方法
    TransactionInterceptor->>DataSourceTransactionManager: getTransaction()
    DataSourceTransactionManager->>DataSource: getConnection()
    DataSource-->>DataSourceTransactionManager: 返回Connection
    DataSourceTransactionManager->>TransactionSynchronizationManager: bindResource(dataSource, connection)
    
    TransactionInterceptor->>Client: 执行业务方法
    Client->>SqlSession: saveBatch()
    SqlSession->>BatchExecutor: doUpdate()
    BatchExecutor->>TransactionSynchronizationManager: getResource(dataSource)
    TransactionSynchronizationManager-->>BatchExecutor: 返回绑定连接
    BatchExecutor->>Connection: 准备Statement并缓存
    
    loop 批处理循环
        Client->>SqlSession: 继续添加批处理
    end
    
    TransactionInterceptor->>DataSourceTransactionManager: commit()
    DataSourceTransactionManager->>BatchExecutor: flushStatements()
    BatchExecutor->>Connection: executeBatch()
    ...
    DataSourceTransactionManager->>DataSource: releaseConnection()